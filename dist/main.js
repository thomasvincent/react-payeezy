...
var ReactPayeezy = function (_React$Component) {
  _inherits(ReactPayeezy, _React$Component);

  function ReactPayeezy(props) {
    _classCallCheck(this, ReactPayeezy);

    var _this = _possibleConstructorReturn(this, (ReactPayeezy.__proto__ || Object.getPrototypeOf(ReactPayeezy)).call(this, props));

    _this.onScriptLoaded = function () {
      if (!ReactPayeezy.PayeezyHandler) {
        ReactPayeezy.PayeezyHandler = Payeezy.configure({
          key: _this.props.PayeezyApiKey
        });
        if (_this.hasPendingClick) {
          _this.showPayeezyDialog();
        }
      }
    };
    ...
    _this.getConfig = function () {
      return ['PayeezyApiKey', 'PayeezyAuthorization', 'PayeezyNonce', 'PayeezyTimestamp', 'token', 'image', 'name', 'description', 'amount', 'locale', 'currency', 'panelLabel', 'zipCode', 'shippingAddress', 'billingAddress', 'email', 'allowRememberMe'].reduce(function (config, key) {
    ...
    _this.onClick = function () {
      ...
      if (scriptDidError) {
        try {
          throw new Error('Tried to call onClick, but Payeezy failed to load');
        } catch (x) {} // eslint-disable-line no-empty
      } else if (ReactPayeezy.PayeezyHandler) {
        _this.showPayeezyDialog();
      } else {
  ...
  _createClass(ReactPayeezy, [{
    ...
    value: function componentDidMount() {
      ...
      script.src = 'https://github.com/payeezy/payeezy_js/blob/master/javascript/payeezy_us_v5.1.js';
      script.async = 1;
    ...
    value: function componentWillUnmount() {
      ...
      if (ReactPayeezy.PayeezyHandler && this.state.open) {
        ReactPayeezy.PayeezyHandler.close();
      }
    }
  }, {
    key: 'updatePayeezyHandler',
    value: function updatePayeezyHandler() {
      if (!ReactPayeezy.PayeezyHandler || this.props.reconfigureOnUpdate) {
        ReactPayeezy.PayeezyHandler = Payeezy.configure({
          key: this.props.PayeezyApiKey
        });
      }
    }
  }, {
    ...
    key: 'showPayeezyDialog',
    value: function showPayeezyDialog() {
      this.hideLoadingDialog();
      ReactPayeezy.PayeezyHandler.open(this.getConfig());
    }
  }, {
    key: 'renderDefaultPayeezyButton',
    value: function renderDefaultPayeezyButton() {
    ...
    value: function render() {
      ...
      return this.props.disabled ? this.renderDisabledButton() : this.renderDefaultPayeezyButton();
    }
  }]);
...
ReactPayeezy.propTypes = {
  // Opens / closes the  modal by value
  ...
  // (Requires including Payeezy-.css or adding the .styl file
  // to your pipeline)
  ...
  // Payeezy.configure, which may result in additional XHR calls to the
  // Payeezy API.  If you know the first configuration is all you need, you
  // can set this to false.  Subsequent updates will affect the Payeezy.open
  // (e.g. different prices)
  reconfigureOnUpdate: _react2.default.PropTypes.bool,

  // =====================================================
  // Required by Payeezy
  // see Payeezy docs for more info:
  //   https://developer.payeezy.com/payeezy-api/
  // =====================================================

  // Your publishable key (test or live).
  // can't use "key" as a prop in react, so have to change the keyname
  PayeezyApiKey: _react2.default.PropTypes.string.isRequired,

  // HMAC of payload signed using api secret
  PayeezyAuthorization: _react2.default.PropTypes.string.isRequired,

  // Epoch time stamp aka. unix time
  PayeezynNonce: _react2.default.PropTypes.string.isRequired,

  // Secure random number
  PayeezyTimestamp: _react2.default.PropTypes.string.isRequired,

  //This is used to identify the merchant
  Payeezyt: _react2.default.PropTypes.string.isRequired,
  ...
  // Specify auto to display  in the user's preferred language, if
  // available. English will be used by default.
  //
  locale: _react2.default.PropTypes.oneOf(['auto', // (Default) Automatically chosen by
  'zh', // Chinese
  'nl', // Dutch
  'en', // English
  'fr', // French
  ...
  // The label of the payment button in the  form (e.g. “Subscribe”,
  // “Pay {{amount}}”, etc.). If you include {{amount}}, it will be replaced
  // by the provided amount. Otherwise, the amount will be appended to the
  // end of your label.
  panelLabel: _react2.default.PropTypes.string,

  // Specify whether  should validate the billing ZIP code (true or
  // false)
  zipCode: _react2.default.PropTypes.bool,

  // Specify whether  should collect the user's billing address
  // (true or false). The default is false.
  billingAddress: _react2.default.PropTypes.bool,

  // Specify whether  should collect the user's shipping address
  // (true or false). The default is false.
  ...
  // function() The callback to invoke when  is opened (not supported
  // in IE6 and IE7).
  opened: _react2.default.PropTypes.func,

  // function() The callback to invoke when  is closed (not supported
  // in IE6 and IE7).
  closed: _react2.default.PropTypes.func
};
exports.default = ReactPayeezy;
